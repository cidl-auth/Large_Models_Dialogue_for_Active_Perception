...
def compute_gradcam(model, visual_input, text_input, tokenized_text, block_num=6):
    ...
    with torch.no_grad():
        ...
        ## Calculate sizes ORIGINAL
        # print(f'Mask size: {mask.size()}')
        # print(f'Cams size before reshape: {cams.size()}')
        # print(f'Grads size before reshape: {grads.size()}')
        cams = cams[:, :, :, 1:].reshape(visual_input.size(0), 12, -1, 24, 24) * mask ##original
        # cams = cams[:, :, :, 1:].reshape(visual_input.size(0), 12, -1, 24, 24) * mask[:, :, :, 1:, 0]
        # print(f'Cams size after operation: {cams.size()}')
        ...
        # print(f'Grads size after operation: {grads.size()}')
##==============================================================================================
