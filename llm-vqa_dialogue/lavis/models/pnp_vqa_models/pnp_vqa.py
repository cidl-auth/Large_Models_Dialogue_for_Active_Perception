...
class PNPVQA(BaseModel):
    ...
    def forward_itm(self, samples, block_num=7):
        ...
        # ORIGINAL GRADCAM SCRIPT BEFORE CHANGES
        #========================================================================================================
        image = samples['image']
        ...
        while min_num_captions < num_captions:
            ...
            for counter, output in enumerate(outputs):
                ...
                if len(captions[ind]) < num_captions:
                    ...
                    ## PROBABILITY TEST 
            ##============================================================================================
                        # logits = decoder_out[counter].float()
                        # softmax_prob = logits[counter].softmax(dim=-1).tolist()
                        # softmax_probs.append(softmax_prob)
            ##============================================================================================

            min_num_captions = min([len(i) for i in captions])

        samples['captions'] = captions + softmax_probs
    ...
    def forward_qa(
        ...
        for question_caption in question_captions_chunk:
            ...
            for output in outputs:
                ...
                output_float = output.to(torch.float64)
    ...
    def predict_answers(
        ...
        ##=====================================================================================================
        ## USE ITM TO SEE HOW GOOD THE CAPTION IS 
        caption = samples['captions'][0][0]
        itm_caption = {"image": samples["image"], "text_input": caption}
        itm_probabilities_caption = self.image_question_matching_model.forward(itm_caption, match_head="itm")
        temp = 2.0
        itm_probabilities_caption = F.softmax(itm_probabilities_caption/temp, dim=1)
        caption_confidence = itm_probabilities_caption[0][1]
        # print(f"ITM PROB (Caption-Image): {itm_probabilities_caption}")
        answer = samples['question_captions'][0]
        itm_answer = {"image": samples["image"], "text_input": answer}
        itm_probabilities_answer = self.image_question_matching_model.forward(itm_answer, match_head="itm")
        itm_probabilities_answer = F.softmax(itm_probabilities_answer, dim=1)
        answer_confidence = itm_probabilities_answer[0][1]
        ##=====================================================================================================

        return pred_answers, samples['captions'], samples['gradcams'], caption_confidence
    ...
    def validate_confidence(
        ...
        itm_custom_caption = {"image": samples["image"], "text_input": caption}
        itm_probabilities_caption = self.image_question_matching_model.forward(itm_custom_caption, match_head="itm")
        temp = 2.0
        itm_probabilities_caption = F.softmax(itm_probabilities_caption/temp, dim=1)
        caption_confidence = itm_probabilities_caption[0][1]

        return caption_confidence
##==============================================================================================================================
    @classmethod
    def from_config(cls, model_config):
        ...
        # print(f"Path of image_question_matching_model class: {inspect.getfile(itm_cls)}")
